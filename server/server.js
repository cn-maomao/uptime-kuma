    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("editMonitor", async (monitor, callback) => {
            try {
                ...
                bean.hllRconPassword = monitor.hllRconPassword;
                bean.hllMinPlayersEnabled = Boolean(monitor.hllMinPlayersEnabled);
                bean.hllMinPlayers = Number.isFinite(parseInt(monitor.hllMinPlayers, 10)) ? parseInt(monitor.hllMinPlayers, 10) : 0;
                bean.hllExitEnabled = Boolean(monitor.hllExitEnabled);
                bean.hllExitDrop = Number.isFinite(parseInt(monitor.hllExitDrop, 10)) ? parseInt(monitor.hllExitDrop, 10) : 0;
                bean.hllExitWindowSec = Number.isFinite(parseInt(monitor.hllExitWindowSec, 10)) ? parseInt(monitor.hllExitWindowSec, 10) : 300;
